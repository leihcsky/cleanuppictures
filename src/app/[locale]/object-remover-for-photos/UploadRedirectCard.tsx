...
export default function UploadRedirectCard({ locale }) {
  ...
  const sampleUrls = [
    'https://pub-08705f8dc4354c6ca3fbd77c36fcec23.r2.dev/remove-object/sample1-remove-object-before.jpg',
    'https://pub-08705f8dc4354c6ca3fbd77c36fcec23.r2.dev/remove-object/sample3-remove-object-before.jpg',
    'https://pub-08705f8dc4354c6ca3fbd77c36fcec23.r2.dev/remove-object/sample4-remove-object-before.jpg'
  ];

  const gotoHome = () => {
    pushToolHome(hrefWithSearchParams(getLinkHref(locale, ''), { mode: 'object' }));
  };
  ...
  const loadSample = (remoteUrl: string) => {
    pushToolHome(hrefWithSearchParams(getLinkHref(locale, ''), { mode: 'object', sample: remoteUrl }));
  };
      ...
      <div className="mt-4 flex items-center justify-center gap-3 flex-wrap">
        {sampleUrls.map((url, idx) => (
          <button key={idx} onClick={() => loadSample(url)} className="rounded-xl border border-slate-200 bg-white p-1.5 hover:border-primary-300 hover:shadow-md transition-all">
            <img src={getImageProxyHref(locale, url)} alt={`Sample ${idx + 1}`} className="w-24 h-16 object-cover rounded-lg" />
          </button>
        ))}
      </div>
    </div>
